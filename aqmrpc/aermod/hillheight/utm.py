...
'''
This module provides datum conversion from degrees (lat, lon) to UTM.
More information: http://www.uwgb.edu/dutchs/usefuldata/utmformulas.htm

Usage:
- use clean_latlon() untuk obtain coordinate in format: 
  (-180 < lon <= 180) and (-90 < lat < 90) 
- use get_zone_hem(lat, lon) to determine UTM Zone and hemisphere of 
  a particular coordinate, hem --> N: north, S: south
- use convert_to_utm(lat, lon) to obtain easting and northing in UTM system
- use convert_to_utm_fixzone(lat, lon, zone, hem) to obtain easting and
  northing of particular UTM zone
- use convert_to_latlon(easting, northing, zone, hem) to obtain
  latitude dan longitude from a UTM coordinate

Notes:
Please use floats instead of ints at all time.
...
def get_zone_hem(lat, lon):
    ''' Return utm zone and hemisphere. '''
    
    lat, lon = clean_latlon(lat, lon)

    lon_x = abs(lon + 180)
    
    zone = int(lon_x / 6.0) + 1
    hem = 'N' # North
    if lat < 0:
        hem = 'S' # South
    if lon == -9999.0:
...
def convert_to_utm_fixzone(lat, lon, zone, hem):
    ...
    p = lon_rad - lon_cent
    #central meridian of the particular UTM zone
...
def convert_to_latlon(easting, northing, zone, hem):
    ''' Conversion from utm to lat/lon '''
    ...
    tmp_7 = (easting - 500000.0) / (tmp_5 * kk)
